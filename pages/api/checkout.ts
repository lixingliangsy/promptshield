...
function withUtm(url: string): string {
  try {
    ...
    u.searchParams.set('utm_content', (PRODUCT as any).slug)
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const cycle =
    req.query && req.query.cycle === 'yearly' && (PRODUCT as any).yearlyProductId ? 'yearly' : 'monthly'
  const productId = cycle === 'yearly' ? (PRODUCT as any).yearlyProductId : (PRODUCT as any).productId
  ...
  if (productId) {
    ...
    console.warn('[checkout] missing productId for', (PRODUCT as any).slug)
  }
  ...
  if (storeBase) {
    const target = storeBase.endsWith('/') ? storeBase + (PRODUCT as any).slug : storeBase + '/' + (PRODUCT as any).slug
    return res.redirect(302, target)
  }
  return res.redirect(302, (PRODUCT as any).checkoutUrl || '/#pricing')
}
